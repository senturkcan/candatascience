...
# if __all__ will not be added, everything on this module will avaliable when the module is called without alias.
# And this moule automatically called because __init__.py has this line:
#from .utils import *

#imports written seperatly so functions and classes can be copied seperatly.
import pandas as pd
import numpy as np
...
def extract(label_column = None, dataset_path="", dataset_full_path="", file_type=None, 
    ...
    try:
        ...
        raise RuntimeError(f"Error: Dataset is too large to load into memory. "
                         f"File: '{file_path}'. "
                         f"Consider using Dask, chunking, sampling, or a more memory-efficient format like Parquet. ")
